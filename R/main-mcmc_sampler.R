...
#' @param init.seed An optional integer specifying the initial seed for random number generation to ensure reproducibility.
...
mcmc_sampler<- function(Y,
  ...
  if (samples.store > (N.MCMC - burn_in1 - burn_in2)) {
    stop(paste0("Error: `samples.store` (", samples.store,
  ...
  if(no.rm.obs>length(A)){
    stop(paste0("Error: `no.rm.obs` (", no.rm.obs,
 ...
 #browser()
  ...
  hyper_fixed<- list("kappa_eta" = c(0.75, 3), # kappa_eta ~ Gamma(shape=hyper_fixed[1], rate=hyper_fixed[2])
                     "kappa_w1" = c(0.75, 3/(m.bar*0.7^2)), # kappa_w1 ~ Gamma(shape=hyper_fixed[3], rate=hyper_fixed[4])
                     "kappa_w2"  = c(0.75, 3/(m.bar*0.7^2)), # kappa_w2 ~ Gamma(shape=hyper_fixed[5], rate=hyper_fixed[6])
                     "kappa_mu" = c(0.75, 3), # kappa_mu ~ Gamma(shape=hyper_fixed[7], rate=hyper_fixed[8])
                     "beta" = 0.01,     # beta ~ norm(mean=0, precision=hyper_fixed[13]))
                     "beta1" = 0.01,     # beta1 ~ mvtnorm_p(mean=0, precision=hyper_fixed[14] I_p))
                     "beta2" = 0.01,     # beta2 ~ mvtnorm_q(mean=0, precision=hyper_fixed[15] I_q))
 ...
 if(model_type=="FE"){
   ...
   results_JM<- mcmc_sampler_joint_model_jSp(N.MCMC = N.MCMC,
                                            Y = Y_data_frame_count$Y_with_NA,
                                            ind_NA_Y=is.na(Y_data_frame_count$Y_with_NA),
                                            A = A_data_frame_size$A_with_NA,
                                            ind_NA_A=is.na(A_data_frame_size$A_with_NA),
...
results<- list("thr.info" =  list(threhold= threshold, thr.acces.ind = thr.acces.ind),
               ...
               "thr.family" = thr.family,
               "model_type" =  model_type,
               "data" = list("count.full" = Y_data_frame_count$Y_with_NA,
